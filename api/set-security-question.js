// API endpoint to set security question for existing accounts (one-time only)
...
export default async function handler(req, res) {
    ...
    try {
        const { playerName, password, securityQuestion, securityAnswer } = req.body;
        ...
        // Check if already has security question (one-time only)
        if (player.security_question && player.security_question.trim() !== '') {
            return res.status(400).json({ error: 'Security question already set. Cannot change.' });
        }

        // Encrypt and save security question/answer
